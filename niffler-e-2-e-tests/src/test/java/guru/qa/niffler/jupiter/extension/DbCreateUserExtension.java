...
public class DbCreateUserExtension extends CreateUserExtension {
    ...
    protected List<UserJson> createFriendsIfPresent(GenerateUser annotation, UserJson currentUser) {
        ...
        if (friends.handleAnnotation()) {
            UserDataEntity currentUserEntity = userRepository
                    .getUserData(AuthUserEntity.fromUserJson(currentUser));
            for (int i = 0; i < friends.count(); i++) {
                AuthUserEntity authUser = getAuthUserEntity();
                userRepository.createUserForTest(authUser);
                UserDataEntity friendUserEntity = userRepository.getUserData(authUser);
                currentUserEntity.addFriends(false, friendUserEntity);
                friendUserEntity.addFriends(false, currentUserEntity);
                userRepository.updateUserForTest(friendUserEntity);
                userJsonList.add(UserJson.fromEntity(friendUserEntity));
            }
